...
N_{i,h} & = f_k((N_{i,h-1} \oplus g(s_0))||(N_{i+1,h-1} \oplus g(s_1))) || s \\
...
\begin{split}
P & = (P_{i,0},...,P_{0,H-1})\\
%P_{i,h} & = (P_{i,0},...,P_{0,H-1})\\
A & = (A_0,...,A_{H-1})\\
%A_h & = 
%\begin{cases}
%f_k(P_{h-1}||a_{h-1} \oplus s), & \textrm{if}\floor{\frac{i}{2^h}} \equiv 0 \textrm{ mod }2;\\
%f_k(a_{h-1}||P_{h-1} \oplus s), & \textrm{if}\floor{\frac{i}{2^h}} \equiv 1 \textrm{ mod }2;
%\end{cases}
%\\P_0 & = N_{i,0}
\end{split}
\end{equation}
...
Briefly talk about where and the idea for Lamport+ came about. Why are random values XORed? Key Scheduling and smaller key sizes\\ \\
