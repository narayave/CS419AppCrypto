...
Lamport+ chains need to be passed in for trees.\\
...
\end{itemize}
\textbf{\underline{Key Generation}}\\
Leaf nodes need to calculated first, so Lamport+ Hash Chains should already exist, and there should be $2^H$ number of chains to make things easy.
\textbf{Input:} $2^H$ Lamport+ hash chains\\
\textbf{Output:} Merkle Hash Tree with Lamport+
\begin{equation}
\begin{split}
0 & = 0
\end{split}
