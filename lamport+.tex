...
\textbf{Chaining Function: }The chaining function $c$ is $c : \{0,1\}^{n+m} \rightarrow \{0,1\}^{n}$, and it's meant to aid with the Lamport+ signature scheme. The subscript, assuming $x \in \{0,1\}^n$, indicates a subset of bits from $x$. $x_{a,b}$ indicates working with bits $x_a,...,x_b$.

\begin{equation}
\begin{split}
\textrm{Iteration: }c^{i}(x) & = f_{k}(x_{1,n} \oplus g(x_{n+1,n+m})) \\
\textrm{Base case: }c^0(x) & = x_{1,n}
\end{split}
\end{equation}

\subsection*{Key Generation}
\vspace{-0.2cm}\hspace{0.5cm}\textbf{Input:} Security parameter n
\mathcenter
\begin{equation}
\begin{split}
sk & = ((x_{1}^0, x_{1}^{1}),...,(x_{n}^0, x_{n}^{1}))\\
& = ((c^0_1(x)||s_0,c^0_1(x)||s_1),...(c^0_n(x)||s_0,c^0_n(x)||s_1))\\
pk & = ((y_{1}^0, y_{1}^{1}),...,(y_{n}^0, y_{n}^{1})) \\ 
& = ((c^1_1(x_{1}^0)||s_0,c^1_1(x_{1}^1)||s_1),...(c^1_n(x_{n}^0)||s_0,c^1_n(x_{n}^0)||s_1))\\
\end{split}
\end{equation}
\hspace{0.5cm}\textbf{Output:} Secret key $sk$, Public key $pk$

\subsection*{Signature Generation}
\vspace{-0.2cm}\hspace{0.5cm}\textbf{Input:} Secret key $sk$, hashed message $M$
\mathcenter
\begin{equation}
\begin{split}
\sigma & = (x^{M_1}_1, ..., x^{M_n}_n), M_i \in \{0,1\} \\
\end{split}
\end{equation}
\hspace{0.5cm}\textbf{Output:} Signature $\sigma$

\subsection*{Signature Verification}
\vspace{-0.2cm}\hspace{0.5cm}\textbf{Input:} Public key $pk$, hashed message $M$, and signature $\sigma$.
\mathcenter
\begin{equation}
\begin{split}
pk & = (y^i_1,...,y^i_n) \\
& \stackrel{?}{=} (c^1_1(x_{1}^i)||\sigma_{n+1,n+m},...,c^1_1(x_{n}^i)||\sigma_{n+1,n+m})
\end{split}
\end{equation}
\hspace{0.5cm}\textbf{Output:} Verification pass or fail\\ \\
