...
The Lamport+ Hash Tree incorporates both the Lamport+ hash chains, and Merkle Hash Trees. The scheme shown here is loosely based on XMSS trees from [6]. The hash tree allows us to sign 
...
\textbf{Output: Authentication path A, signature $\sigma$, index i}
\begin{equation}
\begin{split}
 a_h =
 \begin{cases}
 1, & \textrm{if}\floor{\frac{i}{2^h}} \equiv 0 \textrm{ mod }2\\
 0, & \textrm{if}\floor{\frac{i}{2^h}} \equiv 1 \textrm{ mod }2
 \end{cases}
\end{split}
\end{equation}
\textbf{\underline{Signature Verification}}\\
